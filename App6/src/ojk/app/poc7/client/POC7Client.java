...
public class POC7Client {
	...
	public int sendData(String username, String passwordStr, String operation,
		...
		SOAPElement soapElement = soapBody
				.addChildElement(operationName, "ns1");
		SOAPElement element0 = soapElement.addChildElement("POC7");
	...
	public static POC7 getObj(SOAPMessage soapResponse) throws Exception {

		Node dataBody = soapResponse.getSOAPPart().getEnvelope().getBody()
				.getElementsByTagName("POC7").item(0);
		if (dataBody == null) {
			ByteArrayOutputStream output = new ByteArrayOutputStream();
			soapResponse.writeTo(output);
			String strMsg = new String(output.toByteArray());
			...
			JAXBContext jc = JAXBContext.newInstance(POC7.class);
			Unmarshaller unmarshaller = jc.createUnmarshaller();
			JAXBElement<POC7> je = unmarshaller.unmarshal(dataBody, POC7.class);
			return je.getValue();
		}
